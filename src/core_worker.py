...
def write_jwlibrary(documentId, articleId, title, questions, notes, telegram_user, textareas):
    ...
    if os.path.isfile(uploadedJwLibrary):
        ...
        fileName = "userBackups/{0}/askjw-{1}-{2}.jwlibrary".format(telegram_user, documentId, now_date)
        ...
        fileName = "userBackups/{0}/askwt-{1}-{2}.jwlibrary".format(telegram_user, documentId, now_date)
        with zipfile.ZipFile(fileName, "w", compression=zipfile.ZIP_DEFLATED) as zf:
...
def write_docx_pdf(documentId, title, questions, notes, telegram_user):
    ...
    fileNameDoc = "userBackups/{0}/askwt-{1}-{2}.docx".format(telegram_user, documentId, now_date)
    document.save(fileNameDoc)

    fileNamePDF = "userBackups/{0}/askwt-{1}-{2}.pdf".format(telegram_user, documentId, now_date)
